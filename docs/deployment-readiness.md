...
| `CORS_ALLOWED_ORIGINS` | Optional | Comma-separated absolute `http(s)` origins allowed to call the API directly from browsers. Leave unset to deny cross-origin browser access by default. Production example: `https://auren-workspace.com,https://byte.dns-parking.com,https://pixel.dns-parking.com` |
